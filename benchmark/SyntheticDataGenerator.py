...
class SyntheticDataGenerator:
    ...
    def _random_birth_date(self) -> datetime:
        ...
        Since the pipeline validates age >= MIN_AGE, we prioritize age constraint
        over year range for synthetic data to ensure benchmark reliability.
        ...
        # Always generate birth dates that guarantee age >= MIN_AGE
        # This ensures synthetic data passes validation reliably
        today = datetime.now()
        birth_month = random.randint(1, 12)
        birth_day = random.randint(1, 28)  # Safe day that exists in all months
        
        # Generate age between MIN_AGE and MIN_AGE+40 (40-80 years old)
        target_age = MIN_AGE + random.randint(0, 40)
        target_birth_year = today.year - target_age
        
        # Create birth date and verify age calculation
        birth_date = datetime(target_birth_year, birth_month, birth_day)
        
        # Double-check age calculation to ensure it's >= MIN_AGE
        calculated_age = today.year - birth_date.year - ((today.month, today.day) < (birth_date.month, birth_date.day))
        
        # If somehow still invalid (edge case with birth date vs today's date), adjust
        if calculated_age < MIN_AGE:
            # Force a valid birth year
            birth_date = datetime(today.year - MIN_AGE - 1, birth_month, birth_day)
